...
namespace ApartmentManagementSystem
{
    public static class ReportHelper
    {
        ...
        public static string GenerateFinancialReport(List<Payment> payments, List<Lease> leases, List<Tenant> tenants)
        {
            ...
            var totalPayments = payments.Where(p => p.Status == PaymentStatus.Paid).Sum(p => p.Amount);
            var thisMonthPayments = payments.Where(p => p.Status == PaymentStatus.Paid &&
            ...
            var recentPayments = payments.Where(p => p.Status == PaymentStatus.Paid)
